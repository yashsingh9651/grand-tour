            ...
            {activePayment ? (
              <Card className="p-5 border border-slate-200 bg-white rounded-3xl space-y-4">
                <div className="flex justify-between items-start">
                  ...
                  <Badge 
                    className={`rounded-full px-3 py-1 font-black text-[10px] tracking-wider ${
                      activePayment.status === 'COMPLETED' ? 'bg-emerald-100 text-emerald-700' :
            ...
            <div className="flex justify-between items-start">
              ...
              {paymentObj ? (
                <Badge className={`rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider ${
                  paymentObj.status === 'COMPLETED' ? 'bg-green-100 text-green-700' :
                    ...
                    <Card key={doc.id} className="p-3.5 border border-slate-200 bg-white rounded-2xl flex flex-col justify-between space-y-3 shadow-sm">
                      <div className="flex justify-between items-start gap-2">
                        <div className="min-w-0">
                          ...
                          <span className={`text-[9px] font-bold px-1.5 py-0.25 rounded-full mt-1.5 inline-block ${
                            doc.isPublished ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-amber-50 text-amber-700 border border-amber-200'
                          }`}>
                ...
                {journeySteps.map((step: any, index: number) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-3 py-2.5 rounded-2xl text-[11px] font-semibold uppercase tracking-wider transition-all duration-150 ${
                        isActive 
                    ...
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black shrink-0 ${
                          isActive ? 'bg-[#CCFF00] text-black' : 'bg-slate-100 text-slate-600'
                        }`}>{index + 1}</span>
