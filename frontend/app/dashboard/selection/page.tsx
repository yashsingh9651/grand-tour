...
export default function SelectionPage() {
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 text-foreground">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 bg-emerald-500/10 border border-emerald-500/20 px-5 py-3 rounded-2xl shrink-0 self-start md:self-center">
            <div className="w-8 h-8 bg-emerald-500/20 rounded-full flex items-center justify-center">
              <Award className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
            </div>
            <div>
              <p className="text-[9px] font-bold tracking-widest text-emerald-600 dark:text-emerald-400 uppercase leading-none">SELECTION STATUS</p>
              <p className="text-sm font-extrabold text-emerald-650 dark:text-emerald-350 mt-0.5 font-bold">Status: Selected</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            {/* Offer details Card */}
            <Card className="p-8 border border-border bg-card shadow-sm rounded-[2rem] relative overflow-hidden text-foreground">
              {/* Background "OFFER" watermark text */}
              <div className="absolute right-8 top-4 text-[120px] font-black text-slate-100/10 dark:text-zinc-900/10 select-none pointer-events-none tracking-tighter leading-none">
                OFFER
              </div>
              ...
              <div className="relative z-10 space-y-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-foreground">{application?.hotelAssignment ? 'Hospitality Intern' : 'Principal Design Intern'}</h2>
                    <p className="text-sm text-muted-foreground font-medium">{application?.hotelAssignment?.hotel?.name || 'Lumina Global Creative Studio'} • Full-time</p>
                  </div>
                </div>

                <div className="grid grid-cols-3 gap-4 py-6 border-y border-border">
                  <div>
                    <p className="text-[10px] font-bold text-muted-foreground tracking-wider uppercase">START DATE</p>
                  ...
                  <div>
                    <p className="text-[10px] font-bold text-muted-foreground tracking-wider uppercase">DURATION</p>
                    <p className="text-base font-extrabold text-foreground mt-1">
                  ...
                  <div>
                    <p className="text-[10px] font-bold text-muted-foreground tracking-wider uppercase">STIPEND</p>
                    <p className="text-base font-extrabold text-primary mt-1">$3,200 / mo</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <ul className="space-y-3 pl-1">
                    {[
                      ...
                      <li key={idx} className="flex items-start gap-3 text-sm text-foreground font-semibold">
            ...
            {/* Document Card */}
            <Card className="border border-border bg-card shadow-sm rounded-[2rem] overflow-hidden">
              <div className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-foreground">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-bold text-foreground text-base">Official Contract Agreement</h3>
                    <p className="text-xs text-muted-foreground font-semibold mt-0.5">Word Document • 1.4 MB • Updated 2 hours ago</p>
                  </div>
                </div>
                ...
                <a href={application?.hotelAssignment?.hotel?.proposalPdf || "#"} target={application?.hotelAssignment?.hotel?.proposalPdf ? "_blank" : undefined} onClick={(e) => { if(!application?.hotelAssignment?.hotel?.proposalPdf) { e.preventDefault(); toast.success('Contract download started') } }} className="shrink-0">
                  <Button className="bg-primary text-[#1A1A1A] hover:bg-primary/95 rounded-xl h-11 px-5 gap-2 font-bold text-xs uppercase tracking-wider">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="p-8 border border-border bg-zinc-950 dark:bg-zinc-900 text-white space-y-6 relative overflow-hidden">
              ...
              {!isAlreadyAccepted && !isAlreadyRejected ? (
                <div className="space-y-4 pt-2">
                    ...
                    <button 
                      ...
                      className={`flex-1 flex items-center justify-center gap-2 h-12 rounded-xl text-sm font-bold transition-all border ${
                        selectionAction === 'ACCEPT' 
                          ? 'bg-primary border-primary text-[#1A1A1A] scale-[1.02]' 
                          : 'bg-transparent border-zinc-800 text-zinc-300 hover:bg-white/5'
                      }`}
                    >
                    ...
                    <button 
                      ...
                      className={`flex-1 flex items-center justify-center gap-2 h-12 rounded-xl text-sm font-bold transition-all border ${
                        selectionAction === 'REJECT' 
                          ? 'bg-rose-500/15 border-rose-500/30 text-rose-450 scale-[1.02]' 
                          : 'bg-transparent border-zinc-800 text-zinc-300 hover:bg-white/5'
                      }`}
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <Button 
                    ...
                    className="w-full bg-white hover:bg-zinc-100 disabled:bg-zinc-800 text-[#1A1A1A] disabled:text-zinc-500 font-extrabold h-12 rounded-xl tracking-wider text-xs uppercase transition-all mt-2"
                  >
                    {submitting ? <Loader2 className="w-5 h-5 animate-spin mx-auto text-[#1A1A1A]" /> : 'CONFIRM SELECTION'}
                  </Button>
                </div>
              ) : (
                <div className="pt-2">
                  {isAlreadyAccepted ? (
                    <Button 
                      ...
                      className="w-full bg-primary hover:opacity-95 text-[#1A1A1A] font-extrabold h-12 rounded-xl tracking-wider text-xs uppercase transition-all gap-2"
                    >
            ...
            <Card className="p-6 border border-border bg-card space-y-4 text-foreground">
              ...
              <div className="flex items-center gap-3">
                <div className="flex -space-x-3 overflow-hidden">
                  <img 
                    className="inline-block h-10 w-10 rounded-full ring-2 ring-card" 
                  ...
                  <img 
                    ...
                    alt="Peer avatar 3" 
                  />
                  <div className="inline-flex items-center justify-center h-10 w-10 rounded-full bg-primary/20 ring-2 ring-card text-[11px] font-bold text-primary">
                    +12
                  </div>
                </div>
                
                <p className="text-xs font-semibold text-muted-foreground leading-normal flex-1">
            ...
            {/* Tour the HQ Card */}
            <Card className="h-44 border border-border bg-card rounded-[2rem] overflow-hidden relative group cursor-pointer" onClick={() => toast.info('Tour booking available soon!')}>
              <div 
              ...
              <div className="absolute bottom-6 left-6 right-6 z-20 text-white space-y-1 text-left">
                <h4 className="text-lg font-bold tracking-tight">Tour the HQ</h4>
                <p className="text-zinc-300 text-xs font-medium">See where you&apos;ll be working this fall.</p>
              </div>
            </Card>
