...
export default function Payment3Page() {
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      ...
      await paymentService.submit({
        ...
        utrNumber,
        screenshotUrl,
        description: `Payment3 - ${desc} Installment`,
      })
    ...
    return (
      <StudentLayout currentStep={STEP_ID}>
        <div className="max-w-3xl rounded-3xl border border-dashed border-purple-200 bg-purple-50/40 p-8">
          ...
          <p className="mt-2 text-muted-foreground">{STEP_LABEL} will unlock after {PREV_STEP_LABEL}.</p>
          <Button className="mt-5" onClick={() => window.location.href = `/dashboard/${application?.currentStepId || 'application'}`}>Return to Current Step</Button>
        </div>
      </StudentLayout>
    )
  }

  const paymentsList = (application?.payments || []).filter((p: any) => (p.description || '').includes('Payment3'))
  ...
  const paymentConfig = currentStepConfig?.paymentConfig || {
    bankName: 'Curator International Bank', accountName: 'Lumina Academy Global Education',
    accountNumber: '8829 0012 5562 1009', ifsc: 'LMNAGLXX', currency: 'INR',
    reference: '#LA-2024-8891', merchant: 'Lumina Academy Ltd.', qrCodeUrl: '',
  }
  ...
  return (
    <StudentLayout currentStep={STEP_ID} headerContent={
      <div className="text-xs font-bold text-gray-400 uppercase tracking-widest">MANAGEMENT › <span className="text-[#84CC16] font-extrabold">{STEP_LABEL}</span></div>
    }>
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 font-medium max-w-lg">Final payment step before work permit processing begins.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
        <Card className="p-6 border-none shadow-[0_4px_24px_rgba(0,0,0,0.05)] rounded-[1.75rem] bg-white relative overflow-hidden">
          ...
          <div className="border-t border-gray-100 pt-4 space-y-2">
            <div className="flex items-center justify-between text-xs"><span className="text-gray-400 font-semibold">Merchant</span><span className="text-gray-900 font-bold">{paymentConfig.merchant}</span></div>
            <div className="flex items-center justify-between text-xs"><span className="text-gray-400 font-semibold">Reference</span><span className="text-gray-900 font-bold font-mono">{paymentConfig.reference}</span></div>
          </div>
        </Card>
        ...
        <Card className="p-6 border-none rounded-[1.75rem] bg-[#0F172A] text-white relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-5">
              <div><p className="text-[9px] font-bold tracking-widest text-gray-500 uppercase mb-1">ACCOUNT NAME</p><p className="text-sm font-semibold text-white">{paymentConfig.accountName}</p></div>
              <div><p className="text-[9px] font-bold tracking-widest text-gray-500 uppercase mb-1">SWIFT / BIC</p><p className="text-sm font-semibold text-white font-mono">{paymentConfig.ifsc}</p></div>
              <div><p className="text-[9px] font-bold tracking-widest text-gray-500 uppercase mb-1">BANK NAME</p><p className="text-sm font-semibold text-white">{paymentConfig.bankName}</p></div>
              <div><p className="text-[9px] font-bold tracking-widest text-gray-500 uppercase mb-1">CURRENCY</p><p className="text-sm font-semibold text-white">{paymentConfig.currency}</p></div>
            ...
            return (
              <Card key={index} className={`p-5 border-none rounded-[1.75rem] bg-white relative overflow-hidden flex flex-col transition-all duration-300 hover:shadow-[0_8px_32px_rgba(0,0,0,0.07)] ${status === 'LOCKED' ? 'opacity-60' : ''}`}>
                ...
                <div className="border-t border-gray-100 pt-3 flex items-center justify-between text-[10px] font-bold tracking-wider">
                  {status === 'VERIFIED' && <><span className="text-gray-400 uppercase">VERIFIED</span><span className="text-[#84CC16]">RECEIPT.PDF</span></>}
                  ...
                  {(status === 'UNPAID' || status === 'LOCKED') && <><span className="text-gray-400 uppercase">DUE: {inst.dueDate}</span><span className={`uppercase ${status === 'LOCKED' ? 'text-gray-400' : 'text-purple-500'}`}>{status}</span></>}
                </div>
              </Card>
            )
          })}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <Card className="p-6 border-none shadow-[0_4px_24px_rgba(0,0,0,0.04)] rounded-[1.75rem] bg-white">
          <div className="flex items-center gap-2 mb-5"><Clock className="w-4 h-4 text-gray-400" /><h3 className="text-base font-extrabold text-gray-900">Recent Activity</h3></div>
        ...
        {instructionsPdf ? (
          <Card className="p-6 border-none rounded-[1.75rem] bg-white flex flex-col items-center justify-center text-center gap-4">
            <div className="w-14 h-14 bg-purple-50 rounded-2xl flex items-center justify-center"><FileText className="w-7 h-7 text-purple-600" /></div>
            <div><h3 className="font-extrabold text-gray-900 mb-1">Payment Instructions</h3><p className="text-xs text-gray-500">Download the official payment guide for this stage.</p></div>
            <a href={instructionsPdf} target="_blank" rel="noopener noreferrer">
              <Button className="gap-2 bg-purple-600 hover:bg-purple-700 text-white rounded-xl px-6"><Download className="w-4 h-4" /> Download PDF</Button>
            </a>
          </Card>
        ) : (
          <Card className="p-7 border-none rounded-[1.75rem] relative overflow-hidden bg-[#5B21B6] text-white min-h-[180px]">
            ...
            <div className="relative z-10">
              <div className="w-9 h-9 bg-white/15 rounded-xl flex items-center justify-center mb-4"><ShieldCheck className="w-5 h-5 text-white" /></div>
              <h3 className="text-xl font-extrabold text-white mb-2">Encrypted Security</h3>
              <p className="text-sm text-purple-200 font-medium leading-relaxed max-w-xs">All transactions are processed through 256-bit SSL encryption.</p>
            </div>
          </Card>
        )}
      </div>
      ...
      {isPaymentModalOpen && activeInstallmentIndex !== null && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm overflow-y-auto animate-in fade-in duration-200">
          <div className="relative max-w-lg w-full bg-white rounded-[2.5rem] shadow-2xl flex flex-col p-8 space-y-6 my-8">
      ...
      <UploadPopup isOpen={isUploadOpen} onClose={() => setIsUploadOpen(false)} onUploadComplete={handleUploadComplete} token={token} applicationId={application?.id} documentType="payment_receipt" documentName={`Payment3 Receipt - ${activeInstallmentIndex === 0 ? '1st' : activeInstallmentIndex === 1 ? '2nd' : '3rd'} Installment`} />
    </StudentLayout>
  )
}
